...
class ClingoBackend:
    ...
    def upload_file(self):
        ...
        """
        filename = next((c.value for c in self._context if c.key == "filename"), None)
        content = next((c.value for c in self._context if c.key == "filecontent"), None)
        if not filename:
            raise RuntimeError("No file name found in context")

        if not content:
            raise RuntimeError("No content found in context")

        original_name = next(
            (c.value for c in self._context if c.key == "_filename"), filename
        )
        original_name = original_name.strip('"')

        existing_files = {Path(f).name for f in self._optional_files} | set(
            self._uploaded_files.keys()
        )
        ...
        self._messages.append(("", f"Uploaded file {unique_name}", "success"))
    ...
    def remove_file(self, filename):
        ...
        self._reload_domain()
        self._messages.append(("", f"Removed file {name}", "success"))
